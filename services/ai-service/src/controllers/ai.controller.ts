...
export const generatePR = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    const { vulnerabilityIds, title, description, repositoryUrl, owner, repo, installationId } = req.body;
    ...
    const pr: AutoPR = await createPullRequest(vulnerabilityIds, title, description, { owner, repo, installationId });
